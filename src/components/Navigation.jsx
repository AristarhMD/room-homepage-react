...
export default function Navigation() {
  ...
  const handleMobPress = () => setIsOpen(false);
  ...
  return (
        <header className="absolute z-1 top-12 left-6 md:top-12 md:left-16">
      <nav className="w-full">
        <div className="flex items-center gap-28 md:gap-67">
          <button
            ref={menuIconRef}
          ...
          <div className="flex items-center gap-14">
            ...
            <ul className="hidden xl:flex items-center gap-8" ref={listRef}>
              {["home", "shop", "about", "contact"].map((item, id) => (
                <li key={id} className="desk-nav-item">
                  <a>{item}</a>
                </li>
              ))}
            </ul>
        ...
        {/* Overlay */}
        <div
          onClick={handleMobPress}
          className={`fixed xl:hidden bg-black/50 inset-0 top-0 w-full transition-all duration-300 z-1 ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}
        >
          {/* Mob nav */}
          <div className="bg-white py-12 px-6 md:py-12 md:px-16 flex items-center justify-between w-full">
            <button
              onClick={() => setIsOpen(!isOpen)}
            ...
            <ul className="flex items-center gap-8" ref={listRef}>
              {["home", "shop", "about", "contact"].map((item, id) => (
                <li key={id} className="mob-nav-item">
                  <a>{item}</a>
                </li>
              ))}
            </ul>
